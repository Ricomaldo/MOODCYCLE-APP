...
const useCycleStore = create(
  persist(
    (set, get) => ({
      ...
      updateCycleData: (data) => set((state) => ({
...
// assets/images/melune/emma.png (moderne, jeune)
// assets/images/melune/laure.png (professionnelle)
// assets/images/melune/sylvie.png (maternelle)
// assets/images/melune/christine.png (sage)
// assets/images/melune/clara.png (enthousiaste)
```
...
export class PersonaEngine {
  ...
  static calculateCompatibility(age, preferences, traits) {
    ...
    const prefScore = this.calculatePreferenceMatch(preferences, traits.strongPreferences)
    score += prefScore * 0.5
...
export class ContentManager {
  ...
  static getCachedInsights(persona, phase) {
    ...
    const bundledInsights = require('../data/insights.json')
    return bundledInsights
      .filter(insight => 
        insight.persona === persona && 
        insight.phase === phase
      )
...
*Architecture offline-first pour expérience fluide toujours disponible* 